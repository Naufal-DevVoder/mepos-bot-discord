  ...
  run: async (bot, message, args) => {
    ...
    if (args[0]) {
      ...
      let embed = new MessageEmbed()
        ...
        .setDescription(
          "Global prefix is `/`" +
            `\nServer prefix is \`${prefix ||
              "None"}\`\nIf you have suggestion to me you can type \`/suggestion <text>\`\nIf you find bug, you can report it to my developer by type \`/report <text>\``
        )
        .addFields({
          name: "Support Server",
          value: "`Coming Soon`",
        ...
        .addFields({
          name: "Help Command Usage",
          value: "`help [name command or module]`",
          inline: true
        })
        .addFields({
          name: "❯ Command's Name",
        ...
        .addFields({
          ...
          value: `${command.description || "Not provided"}`,
          inline: true
        })
        .addFields({
          name: "❯ Command's Usage",
          value: "`" + `${command.usage || "Not provided"}` + "`",
          inline: true
        })
        .addFields({
          name: "❯ Command's Aliases",
          value: "`" + `${command.aliases || "No provided"}` + "`",
          inline: true
        })
